...
const ProfileRoute = ({ avatar, userName, time, userId, height, width, link, textFont, isHover }) => {
      ...
      <>
        <Link to={profileLink}>
          <div className={`flex items-center gap-2 cursor-pointer p-2 
            ${isHover ? "hover:bg-[var(--hover-color)] hover:rounded-lg transition duration-150 ease-in-out" : ""} `}
          >
            <img
              className={`${height || "h-11"} ${
                width || "w-11"
